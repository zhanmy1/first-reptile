...
def total_array(list,newfeature_id_list,type_id_list):
    result = ['',0,0,0,0,0]
    ary = np.array(list)
    flag = False
    for i in range(ary.shape[1]):
        no = ary[0,i]
        if not flag and newfeature_id_list.count(no)>0:
            result[0] = no
            flag = True
        if flag and (not type_id_list or type_id_list.count(no)>0):
            column = ary[:, i]
            result[1] += int(column[1])
            result[2] += int(column[2])
            result[3] += int(column[1])+int(column[2])
            result[4] += 1
    return result
