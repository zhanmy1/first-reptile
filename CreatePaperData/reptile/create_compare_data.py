...
def getResult(project,type):
    ...
    file_and_row = FileUtil.read("statistical_data/comper_file.txt").split("\n")
    files = []
    rows = {}
    for fr in file_and_row:
        line = fr.split(',')
        files.append(line[0])
        rows[line[0]] = line[1]
    ...
    for result in reversed(results):
        ...
        if files.count(old_name)>0:
            old_value[0] = old_name

        #用获取到的key_id进行筛选
        if key_id.count(result[0]):
...
if __name__ == '__main__':
    ...
    data = getResult(conf.now_project, 'Bug')
    FileUtil.import_compare_excel(fileds, data, conf.now_project, 'Bug')
