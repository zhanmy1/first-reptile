...
def get_data(interval,project,type):
    #获取月份区间
    """
    ...
    result = GetAPIResult.get_month_data(conf.headers,start_m,end_m,project,type)
    #处理获取到的数据的xml数据字符串
    ...
    if(total>0):
        FileUtil.append("/avro-jira-bugs/"+project+"-"+type+"/",project.upper()+start_m+"-"+end_m+".xml",result)
...
if __name__ == '__main__':
    for project in conf.projects:
        for type in conf.type:
            #获取所有report的总数
            all_data = GetAPIResult.get_all_data(conf.headers,project,type)
            ...
            while True:
                ...
                print(now_total)
                #统计
                now_total += int(get_data(interval,project,type))
