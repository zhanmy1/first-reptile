...
def getResult():
    ...
    for item in commits:
        ...
        for item_file in files_result['files']:
            ...
            """
                setdefault函数是给字典中某一key设置value，如果折耳根key不存在就设置后面的value，如果存在就返回已存
            在的值，所以如果某一key（文件名）已经在我们的字典里面，那么我们直接对这个key对应的value（修改次数）加一
            即可，如果不存在，我们就设置这个新的key（文件名）的value（修改次数）为0，然后因为是第一次获取到，就直接
            再0之上加一即可
            """
